...
def monte_carlo(start_price, days, mu, sigma):
    price = np.zeros(days + 7)  # Initializes price with zeros
    print(len(price))
    price[0] = start_price

    for x in range(1, days + 9):
        total_shock = 0
        for i in range(prev_days): #was initally for days
...
# need 7 different arrays, one for each of the days within the week for predicting the price for X stock
sim1 = np.zeros(num_simulations)
sim2 = np.zeros(num_simulations)
sim3 = np.zeros(num_simulations)
sim4 = np.zeros(num_simulations)
sim5 = np.zeros(num_simulations)
sim6 = np.zeros(num_simulations)
sim7 = np.zeros(num_simulations)
...
for i in range(num_simulations):
    ...
    sim[i] = result[days-1]  # final price is stored #TODO instead need to store final  7 day price here

    sim1[i] = result[(days + 1) - 1]
    sim2[i] = result[(days + 2) - 1]
    sim3[i] = result[(days + 3) - 1]
    sim4[i] = result[(days + 4) - 1]
    sim5[i] = result[(days + 5) - 1]
    sim6[i] = result[(days + 6) - 1]
    sim7[i] = result[(days + 7) - 1]

    plt.plot(result)

# for i in range (days, days + 8):
#     result = monte_


day_1_predicted_price = np.median(np.sort(sim1))
day_2_predicted_price = np.median(np.sort(sim2))
day_3_predicted_price = np.median(np.sort(sim3))
day_4_predicted_price = np.median(np.sort(sim4))
day_5_predicted_price = np.median(np.sort(sim5))
day_6_predicted_price = np.median(np.sort(sim6))
day_7_predicted_price = np.median(np.sort(sim7))

plt.xlabel('Days')
plt.ylabel('Price')
plt.title(f'7 Day Price Prediction of GOOGL')

predicted_prices = [day_1_predicted_price, day_2_predicted_price, day_3_predicted_price,
                    day_4_predicted_price, day_5_predicted_price, day_6_predicted_price, day_7_predicted_price]
